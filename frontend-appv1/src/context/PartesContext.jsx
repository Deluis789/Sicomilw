...
export const PartesProvider = ({ children }) => {
    ...
    const descargarExcelPersonal = async () => {
        try {
            const response = await partesService.downloadReportePersonal();
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'reporte_personal.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
    ...
    const descargarExcelPlanilla = async () => {
        try {
            const response = await partesService.downloadReportePlanillaExcel();
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'planilla_vacaciones.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Error al descargar el reporte de plailla de excel:', error);
            }
    };
