    ...
    public function obtenerVacacionesPersona()
    {
        try {
            ...
            $vacationRecords = DB::table('personas')
                ...
                ->select(
                    'personas.idpersona as id',
                    'tiponovedad.novedad as novedades',
                    'tiponovedad.created_at',
                    DB::raw("CONCAT(grados.abregrado, ' ', personas.appaterno, ' ', personas.apmaterno, ' ', personas.nombres) as nombre")
                )
                ->where('tiponovedad.novedad', 'VACACIÓN')
                ->whereIn('personas.idorg', $accessibleOrganizationIds)
                ->get();

            if ($vacationRecords->isEmpty()) {
                return response()->json([
                    'status' => false,
                    'message' => 'No se encontraron registros de vacaciones para las personas en las organizaciones asignadas al usuario.',
                    'data' => []
                ], 404);
            }
            ...
            return response()->json([
                'status' => true,
                'message' => 'Registros de vacaciones encontrados exitosamente.',
                'data' => $vacationRecords
            ], 200);
        } catch (Exception $e) {
            \Log::error('Error al obtener registros de vacaciones para el usuario ' . $userId . ': ' . $e->getMessage(), ['exception' => $e]);

            return response()->json([
                'status' => false,
                'message' => 'Error interno del servidor al obtener los registros de vacaciones.',
            ], 500);
        }
    }
}
