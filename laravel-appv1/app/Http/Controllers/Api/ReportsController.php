...
use App\Exports\ParteGeneralExport;
    ...
    public function exportPersonasExcel()
    {
        ...
        $personas = DB::table('personas')
            ...
            ->select(
                ...
                'sexos.sexo',
                'armas.arma as arma',
                'statuscvs.name as cv',
                'situaciones.situacion as situacion',
                'expediciones.Departamento as expedicion',
                'organizacion.nomorg as nombre_organizacion',
    ...
    public function planillaVacacionesExcel()
    {
        try {
            ...
            if ($personas->isEmpty()) {
                return response()->json([
                    'status' => false,
                    'message' => 'No se encontraron registros para la gestión actual.'
                ], 404);
            }
    
            return Excel::download(new PlanillaVacacionesExport($personas), 'planilla_vacaciones.xlsx');
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
                'message' => 'Error al generar el reporte: ' . $e->getMessage()
